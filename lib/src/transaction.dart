...
class Input {
  ...
  factory Input.expandInput(Uint8List scriptSig, List<Uint8List?>? witness, [String? type, Uint8List? scriptPubKey]) {
    ...
    if (type == SCRIPT_TYPES['P2SH']) {
      ...
      final expanded = Input.expandInput(redeem.input!, redeem.witness, outputType, redeem.output);
...
class Output extends OutputBase {
  Output({String? type, Uint8List? script, int? value, Uint8List? valueBuffer, List<Uint8List>? pubkeys, List<Uint8List>? signatures, int? maxSignatures})
      : super(type: type, script: script, value: value, valueBuffer: valueBuffer, pubkeys: pubkeys, signatures: signatures, maxSignatures: maxSignatures) {
    if (value != null && !isShatoshi(value)) throw ArgumentError('Invalid output value');
  }
}
