...
final BLANK_OUTPUT = new Output(script: EMPTY_SCRIPT, valueBuffer: Uint8List.fromList(VALUE_UINT64_MAX));
...
class Transaction {
  ...
  bool hasWitnesses() {
    try {
      ins.firstWhere((input) => input.witness != null && input.witness!.length != 0);
      return true;
    } catch (e) {
      return false;
    }
  ...
  hashForWitnessV0(
    ...
    if ((hashType & 0x1f) != SIGHASH_SINGLE && (hashType & 0x1f) != SIGHASH_NONE) {
      var txOutsSize = outs.fold(0, (int sum, output) => sum + 8 + varSliceSize(output.script!));
...
class Input {
  ...
  String? prevOutType;
  bool hasWitness = false; // Default value
  ...
  Input({
    ...
    if (this.value != null && !isShatoshi(this.value!)) throw ArgumentError('Invalid output value');
  }
  ...
  factory Input.clone(Input input) {
    return new Input(
      ...
      pubkeys: input.pubkeys != null ? input.pubkeys!.map((pubkey) => Uint8List.fromList(pubkey != null ? pubkey : [])).toList() : null,
      signatures: input.signatures != null ? input.signatures!.map((signature) => signature != null ? Uint8List.fromList(signature) : null).toList() : null,
    );
  }
...
class Output {
  ...
  factory Output.expandOutput(Uint8List script, [Uint8List? ourPubKey]) {
    ...
    if (type == SCRIPT_TYPES['P2WPKH']) {
      ...
      return new Output(pubkeys: [ourPubKey], signatures: []);
    } else if (type == SCRIPT_TYPES['P2PKH']) {
  ...
  factory Output.clone(Output output) {
    return new Output(
      ...
      pubkeys: output.pubkeys != null ? output.pubkeys!.map((pubkey) => pubkey != null ? Uint8List.fromList(pubkey) : null).toList() : null,
      signatures: output.signatures != null ? output.signatures!.map((signature) => signature != null ? Uint8List.fromList(signature) : null).toList() : null
    );
  }
