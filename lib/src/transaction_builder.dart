...
class TransactionBuilder {
  ...
  int addOutput(dynamic data, int value) {
    ...
    if (!_canModifyOutputs()) throw new ArgumentError('No, this would invalidate signatures');

    return _tx.addOutput(scriptPubKey, value);
  }

  int addInput(dynamic txHash, int vout, [int? sequence, Uint8List? prevOutScript]) {
    if (!_canModifyInputs()) throw new ArgumentError('No, this would invalidate signatures');
  ...
  sign({required int vin, required ECPair keyPair, Uint8List? redeemScript, int? witnessValue, Uint8List? witnessScript, int? hashType}) {
    if (keyPair.network.toString().compareTo(network.toString()) != 0) throw new ArgumentError('Inconsistent network');
    ...
    if (!_canSign(input)) {
      if (witnessValue != null) input.value = witnessValue;
  ...
  Transaction _build(bool allowIncomplete) {
    ...
    for (var i = 0; i < _inputs.length; i++) {
      if (_inputs[i].pubkeys!.length != 0 && _inputs[i].signatures!.length != 0) {
    ...
    if (!allowIncomplete) {
      ...
      if (_overMaximumFees(tx.virtualSize())) throw new ArgumentError('Transaction has absurd fees');
    }
  ...
  bool _canModifyInputs() {
    return _inputs.every((input) {
      if (input.signatures!.length == 0) return true;
  ...
  bool _canModifyOutputs() {
    ...
    return _inputs.every((input) {
      ...
      return input.signatures!.every((signature) {
        ...
        if (hashTypeMod == SIGHASH_SINGLE) return nInputs <= nOutputs;
  ...
  bool _needsOutputs(int signingHashType) {
    if (signingHashType == SIGHASH_ALL) return this._tx.outs.length == 0;
  ...
  bool _canSign(Input input) {
    return input.signatures != null && input.signScript != null && input.pubkeys != null && input.signatures!.length == input.pubkeys!.length && input.pubkeys!.length > 0;
  }

  _addInputUnsafe(Uint8List hash, int vout, Input options) {
    String txHash = HEX.encode(hash);
    Input input;
    if (isCoinbaseHash(hash)) throw new ArgumentError('coinbase inputs not supported');
