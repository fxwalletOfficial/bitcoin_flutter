...
Uint8List compile(List<dynamic> chunks) {
  final bufferSize = chunks.fold(0, (num acc, chunk) {
    if (chunk is int) return acc + 1;
    if (chunk.length == 1 && asMinimalOP(chunk) != null) return acc + 1;

    return acc + encodingLength(chunk.length) + chunk.length;
  });
  ...
  chunks.forEach((chunk) {
    ...
    if (chunk is Uint8List) {
      ...
      EncodedPushData epd = encode(buffer, chunk.length, offset);
      offset += epd.size;
      buffer = epd.buffer;
