...
class P2WPKH {
  ...
  _init() {
    ...
    if (data.output != null) {
      if (data.output!.length != 22 || data.output![0] != OPS['OP_0'] || data.output![1] != 20) throw new ArgumentError('Output is invalid');
      if (data.hash == null) data.hash = data.output!.sublist(2);
    ...
    if (data.witness != null) {
      ...
      if (!bscript.isCanonicalScriptSignature(data.witness![0])) throw new ArgumentError('Witness has invalid signature');
  ...
  void _getDataFromWitness([List<Uint8List>? witness]) {
    ...
    if (data.pubkey == null) {
      ...
      if (data.hash == null) data.hash = hash160(data.pubkey!);
  ...
  void _getDataFromHash() {
    if (data.address == null) data.address = segwit.encode(Segwit(network.bech32!, 0, data.hash!));
    if (data.output == null) data.output = bscript.compile([OPS['OP_0'], data.hash]);
  }

  void _getDataFromAddress(String address) {
    try {
      Segwit _address = segwit.decode(address);
      if (network.bech32 != _address.hrp) throw new ArgumentError('Invalid prefix or Network mismatch');
      if (_address.version != 0) throw new ArgumentError('Invalid address version');
