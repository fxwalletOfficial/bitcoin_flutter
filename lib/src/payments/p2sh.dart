...
class P2SH {
  ...
  P2SH({required this.data, NetworkType? network}) {
  ...
  void _init() {
    ...
    if (data.output != null) {
      if (data.output!.length != 23 || data.output![0] != OPS['OP_HASH160'] || data.output![1] != 0x14 || data.output![22] != OPS['OP_EQUAL']) throw ArgumentError('Output is invalid');

      final hash = data.output!.sublist(2, 22);

      if (data.hash != null && data.hash.toString() != hash.toString()) throw ArgumentError('Hash mismatch');
    ...
    if (data.witness != null) {
      if (data.redeem != null && data.redeem!.witness != null && !_stacksEqual(data.redeem!.witness!, data.witness!)) throw ArgumentError('Witness and redeem.witness mismatch');
    }
  }
  ...
  void _checkRedeem(PaymentData redeem) {
    ...
    if (redeem.output != null) {
      ...
      if (decompile.isEmpty) throw ArgumentError('Redeem.output too short');

      // match hash against other sources
      final hash2 = hash160(redeem.output!);
      if (data.hash != null && data.hash!.isNotEmpty && (data.hash.toString() != hash2.toString())) throw ArgumentError('Hash mismatch');
    }
    ...
    if (redeem.input != null) {
      ...
      if (!hasInput && !hasWitness) throw ArgumentError('Empty input');
      if (hasInput && hasWitness) throw ArgumentError('Input and witness provided');

      if (hasInput) {
        final riChunks = bscript.decompile(redeem.input);
        if (!bscript.isPushOnly(riChunks)) throw ArgumentError('Non push-only scriptSig');
      }
    }
  }
  ...
  void _getDataFromInput() {
    ...
    if (chunks == null || chunks.isEmpty) throw ArgumentError('Input too short');
